...
```bash
STACK_NAME=stage-localhost sh ./scripts/deploy.sh
```
...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.zhanje_local.app-db-data == true
```
...
```bash
docker node update --label-add zhanje_local.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stage-localhost.app-db-data=true cat.example.com
```
...
Frontend: https://stage.localhost

Backend: https://stage.localhost/api/

Automatic Interactive Docs (Swagger UI): https://stage.localhost/docs

Automatic Alternative Docs (ReDoc): https://stage.localhost/redoc

PGAdmin: https://pgadmin.stage.localhost

Flower: https://flower.stage.localhost
